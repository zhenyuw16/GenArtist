...
if __name__ == "__main__":
    ...
    cache.cache_path = 'lmd_benchmark.json'
    cache.init_cache()

    #prompts = get_prompts(args.prompt_type, model=model)
    #print(f"Number of prompts: {len(prompts)}")
    prompts = list(json.load(open(cache.cache_path)).keys() )

    #rr = json.load(open('lmd_benchmark_llama2_13b.json'))
    #rr = json.load(open('lmd_benchmark_llama2.json'))
    #rr = json.load(open('lmd_benchmark_internlm.json'))
    rr = json.load(open('bb70.json'))
    ...
    for ind, prompt in enumerate(tqdm(prompts)):
        ...
        #)
        objs = [{'name': k, 'bounding_box': rr[prompt]['objs'][k]} for k in rr[prompt]['objs']]
        gen_boxes, bg_prompt, neg_prompt = objs, rr[prompt]['background'], ""
