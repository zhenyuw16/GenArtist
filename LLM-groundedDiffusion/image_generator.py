...
from utils import parse, guidance, attn, vis
from utils import latents_new as latents
from utils.latents import get_scaled_latents
# from utils import DEFAULT_SO_NEGATIVE_PROMPT, DEFAULT_OVERALL_NEGATIVE_PROMPT
...
def generate_single_object_with_box(
    ...
    **kwargs,
    ...
    # print(bboxes, phrases, input_latents[0].sum())
    (
        latents,
    ...
    Image.fromarray(single_object_images[0]).save('0.png')
    if return_img:
        return single_object_images[0]
        # return np.array(single_object_pil_image_box_ann)
...
def run_singleobj(
    ...
    add_spec["prompt"] = spec["prompt"]
    add_spec["bg_prompt"] = spec["bg_prompt"]
    add_spec["extra_neg_prompt"] = None #spec["extra_neg_prompt"]
    add_spec["gen_boxes"] = []
    ...
    with torch.autocast("cuda", enabled=use_autocast):
        ...
        # bg_all_latents = (bg_all_latents.cuda() )
        initial_bg = bg_all_latents[0] #.half()
        # added objects
        input_latents_list, latents_bg = latents.get_input_latents_list(
            model_dict,
            bg_seed=bg_seed,
        ...
        if use_ref_ca or frozen_steps > 0:
            ...
            for idx, ((prompt, phrase, word, box), input_latents) in enumerate(
                ...
                single_object_pil_image_box_ann = generate_single_object_with_box(
                    prompt,
                    box,
                    phrase,
                    word,
                ...
                single_object_pil_image_box_ann = Image.fromarray(single_object_pil_image_box_ann)
                #single_object_pil_image_box_ann.save('0.png')
                #print('0.png')

    return single_object_pil_image_box_ann
