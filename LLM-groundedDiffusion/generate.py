...
parser.add_argument("--seed_offset", default=3, type=int, help="Offset to the seed (seed starts from this number)")
...
for regenerate_ind in range(args.regenerate):
    ...
    for prompt_ind, prompt in enumerate(tqdm(prompts, desc=f"Run: {save_dir}")):
        ...
        # Load from cache
        print(cache.global_cache, prompt)
                ...
                for repeat_ind in range(repeats):
                    ...
                    if args.run_model in our_models:
                        ...
                        if args.no_synthetic_prompt:
                            ...
                            print(original_ind_base + ind_offset, ind + ind_offset + LARGE_CONSTANT)
                            # exit()
                            print(run_kwargs)
                            exit()
                            output = run(
