...
class AttendExciteCrossAttnProcessor:
    ...
    def __call__(self, attn, hidden_states, encoder_hidden_states=None, attention_mask=None):
        ...
        # print('aaa', type(attn), query.shape, key.shape)
        query = attn.head_to_batch_dim(query)
        key = attn.head_to_batch_dim(key)
        value = attn.head_to_batch_dim(value)
        # print('aaa', hidden_states.shape)

        attention_mask = attn.prepare_attention_mask(attention_mask, key.shape[1], batch_size=1)

        # if attention_mask is None:
        #     print(type(attn), query.shape, key.shape)
        # else:
        #     print(type(attn), query.shape, key.shape, attention_mask.shape)
...
def register_attention_control(model, controller):
    ...
    for name in model.transformer.attn_processors.keys():
        cross_attention_dim = None if name.endswith("attn1.processor") else model.transformer.config.cross_attention_dim
        # print(name)
...
class AttentionStore(AttentionControl):
    ...
    def forward(self, attn, is_cross: bool):
        # key = f"{place_in_unet}_{'cross' if is_cross else 'self'}"
        key = "cross" if is_cross else 'self'
        print(attn.shape)
        if attn.shape[1] <= 32 ** 2:  # avoid memory overhead
            self.step_store[key].append(attn)
        return attn
...
def aggregate_attention(attention_store: AttentionStore,
    ...
    for location in from_where:
        ...
        for item in attention_maps['cross' if is_cross else 'self']:
