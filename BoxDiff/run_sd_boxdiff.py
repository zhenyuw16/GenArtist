...
def load_model(config: RunConfig):
    ...
    if config.sd_2_1:
        stable_diffusion_version = "../../LAVIS/stable-diffusion-2-1-base"
    else:
        #stable_diffusion_version = "CompVis/stable-diffusion-v1-4"
        stable_diffusion_version = "../stable-diffusion-webui/models/Stable-diffusion/"
...
def main(config: RunConfig):
    ...
    # import pickle
    # pickle.dump(config.bbox, open('a.pkl','wb'))
    bb = config.bbox
    for i in range(len(bb)):
        x1, y1, w, h = bb[i]
        bb[i] = [x1, y1, x1+w, y1+h]
    config.bbox = bb
    ...
    joined_image.save(config.output_path / f'000.png')
